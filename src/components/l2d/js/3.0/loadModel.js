...
import LIVE2DCUBISMFRAMEWORK from './framework/live2dcubismframework.js'
import LIVE2DCUBISMPIXI from './framework/live2dcubismpixi.js'
var LIVE2DCUBISMCORE = Live2DCubismCore
//如果资源在CDN，一定要写http://或者https://否则会以本域名请求且为相对路径
//模型的model3.json文件路径
var modelName = 'dujiaoshou_4'
// var baseModelPath = window.location.protocol+'//'+ window.location.host+"/live2d_on_website/Resource/.../";
var baseModelPath = 'https://l2d.alg-wiki.com/assets'
var modelNames = ["modelname1","modelname2","modelname3","...","..."];
var modelPath = `https://l2d.alg-wiki.com/assets/${modelName}/${modelName}.model3.json`;
//Application全局变量
var app = null;
//模型渲染的位置
var tag_target = '#live2d3';
//待机的动作索引
var idleIndex;
...
function loadMoc(mocPath){
       console.log('moc', modelPath.substr(0, modelPath.lastIndexOf('/') + 1) + mocPath)
...
function loadTextures(textures){
    if(textures.length >0){
        ...
        console.log('texture', modelPath.substr(0, modelPath.substr(0, modelPath.lastIndexOf('/') + 1) + textures[i]))
        }
    }else{
...
function loadPhyPath(phyPath){
    if(typeof(phyPath) !== 'undefined'){
        ...
        console.log('physics', modelPath.substr(0, modelPath.lastIndexOf('/') + 1) + phyPath)
    }else{
...
function loadMotions(motions){
    ...
    if(motions.length >0){
        for (var i = 0; i < motions.length; i++) {
            ...
            console.log('motion'+ ( motionCount + 1) , modelPath.substr(0, modelPath.lastIndexOf('/') + 1) + motions[i].File)
...
function loadModel(option){
    modelName = option.modelName ? option.modelName : modelName
    tag_target = option.tag_target ? option.tag_target : tag_target
    modelWidth = option.modelWidth ? option.modelWidth : modelWidth
    modelHight = option.modelHight ? option.modelHight : modelHight
    ...
    modelPath = `https://l2d.alg-wiki.com/assets/${modelName}/${modelName}.model3.json`;
    //拼接路径
    ...
    // modelPath = 'https://l2d.alg-wiki.com/assets/tianlangxing_3/tianlangxing_3.model3.json';
    // D:/work/l2d-Master/l2dModel/live2DModel/bisimai_2
    var ajax = null;
    ...
    // ajax.open('GET', 'https://l2d.alg-wiki.com/assets/bisimai_2/bisimai_2.model3.json', true);
