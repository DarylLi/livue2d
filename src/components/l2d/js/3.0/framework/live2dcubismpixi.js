...
import Live2DCubismCore from '../core/live2dcubismcore.js'

var __extends = (this && this.__extends) || (function() {
  var extendStatics = function(d, b) { extendStatics = Object.setPrototypeOf || ({ __proto__: [] } instanceof Array && function(d, b) { d.__proto__ = b; }) || function(d, b) { for (var p in b)
        if (b.hasOwnProperty(p)) d[p] = b[p]; }; return extendStatics(d, b); };
  return function(d, b) {
    extendStatics(d, b);
...
(function(LIVE2DCUBISMPIXI) {
  var Model = (function(_super) {
    ...
    function Model(coreModel, textures, animator, physicsRig, userData, groups) {
      ...
      if (_this._coreModel == null) { return _this; }
      ...
      for (var m = 0; m < _this._meshes.length; ++m) {
        ...
        for (var v = 1; v < uvs.length; v += 2) { uvs[v] = 1 - uvs[v]; }
        ...
        if (Live2DCubismCore.Utils.hasBlendAdditiveBit(_this._coreModel.drawables.constantFlags[m])) {
          if (_this._coreModel.drawables.maskCounts[m] > 0) { var addFilter = new PIXI.Filter();
            addFilter.blendMode = PIXI.BLEND_MODES.ADD;
            _this._meshes[m].filters = [addFilter]; } else { _this._meshes[m].blendMode = PIXI.BLEND_MODES.ADD; }
        } else if (Live2DCubismCore.Utils.hasBlendMultiplicativeBit(_this._coreModel.drawables.constantFlags[m])) {
          if (_this._coreModel.drawables.maskCounts[m] > 0) { var multiplyFilter = new PIXI.Filter();
    ...
    Object.defineProperty(Model.prototype, "parameters", { get: function() { return this._coreModel.parameters; }, enumerable: true, configurable: true });
    Object.defineProperty(Model.prototype, "parts", { get: function() { return this._coreModel.parts; }, enumerable: true, configurable: true });
    Object.defineProperty(Model.prototype, "drawables", { get: function() { return this._coreModel.drawables; }, enumerable: true, configurable: true });
    Object.defineProperty(Model.prototype, "canvasinfo", { get: function() { return this._coreModel.canvasinfo; }, enumerable: true, configurable: true });
    Object.defineProperty(Model.prototype, "textures", { get: function() { return this._textures; }, enumerable: true, configurable: true });
    Object.defineProperty(Model.prototype, "animator", { get: function() { return this._animator; }, enumerable: true, configurable: true });
    Object.defineProperty(Model.prototype, "userData", { get: function() { return this._userData; }, enumerable: true, configurable: true });
    Object.defineProperty(Model.prototype, "meshes", {set: function(val) { this._meshes = val }, get: function() { return this._meshes; }, enumerable: true, configurable: true });
    Object.defineProperty(Model.prototype, "masks", { get: function() { return this._maskSpriteContainer; }, enumerable: true, configurable: true });
    Object.defineProperty(Model.prototype, "groups", { get: function() { return this._groups; }, enumerable: true, configurable: true });
    Model.prototype.update = function(delta) {
      ...
      for (var m = 0; m < this._meshes.length; ++m) {
        ...
        if (Live2DCubismCore.Utils.hasVertexPositionsDidChangeBit(this._coreModel.drawables.dynamicFlags[m])) { this._meshes[m].vertices = this._coreModel.drawables.vertexPositions[m];
          this._meshes[m].dirtyVertex = true; }
        if (Live2DCubismCore.Utils.hasRenderOrderDidChangeBit(this._coreModel.drawables.dynamicFlags[m])) { sort = true; }
      }
      if (sort) { this.children.sort(function(a, b) { var aIndex = _this._meshes.indexOf(a); var bIndex = _this._meshes.indexOf(b); var aRenderOrder = _this._coreModel.drawables.renderOrders[aIndex]; var bRenderOrder = _this._coreModel.drawables.renderOrders[bIndex]; return aRenderOrder - bRenderOrder; }); }
      this._coreModel.drawables.resetDynamicFlags();
    };
    Model.prototype.destroy = function(options) {
      if (this._coreModel != null) { this._coreModel.release(); }
      _super.prototype.destroy.call(this, options);
      this.masks.destroy();
      this._meshes.forEach(function(m) { m.destroy(); });
      if (options == true || options.texture) { this._textures.forEach(function(t) { t.destroy(); }); }
    };
    ...
    Model._create = function(coreModel, textures, animator, physicsRig, userData, groups) {
      ...
      if (!model.isValid) { model.destroy(); return null; }
      return model;
    };
    Object.defineProperty(Model.prototype, "isValid", { get: function() { return this._coreModel != null; }, enumerable: true, configurable: true });
    return Model;
  }(PIXI.Container));
  ...
  var MaskSpriteContainer = (function(_super) {
    ...
    function MaskSpriteContainer(coreModel, pixiModel) {
      ...
      for (var m = 0; m < pixiModel.meshes.length; ++m) {
        if (_maskCounts[m] > 0) {
          ...
          for (var n = 0; n < _maskRelationList[m].length; ++n) { var meshMaskID = coreModel.drawables.masks[m][n]; var maskMesh = new CubismMesh(pixiModel.meshes[meshMaskID].texture, pixiModel.meshes[meshMaskID].vertices, pixiModel.meshes[meshMaskID].uvs, pixiModel.meshes[meshMaskID].indices, PIXI.DRAW_MODES.TRIANGLES);
    ...
    Object.defineProperty(MaskSpriteContainer.prototype, "maskSprites", { get: function() { return this._maskSprites; }, enumerable: true, configurable: true });
    Object.defineProperty(MaskSpriteContainer.prototype, "maskMeshes", { get: function() { return this._maskMeshContainers },set: function(val) { this._maskMeshContainers = val;}, enumerable: true, configurable: true });
    MaskSpriteContainer.prototype.destroy = function(options) { this._maskSprites.forEach(function(m) { m.destroy(); });
      this._maskTextures.forEach(function(m) { m.destroy(); });
      this._maskMeshContainers.forEach(function(m) { m.destroy(); });
      this._maskShader = null; };
    MaskSpriteContainer.prototype.update = function(appRenderer) { for (var m = 0; m < this._maskSprites.length; ++m) { appRenderer.render(this._maskMeshContainers[m], this._maskTextures[m], true, null, false); } };
    MaskSpriteContainer.prototype.resize = function(viewWidth, viewHeight) { for (var m = 0; m < this._maskTextures.length; ++m) { this._maskTextures[m].resize(viewWidth, viewHeight, false); } };
    return MaskSpriteContainer;
  }(PIXI.Container));
  LIVE2DCUBISMPIXI.MaskSpriteContainer = MaskSpriteContainer;
  var ModelBuilder = (function() {
    function ModelBuilder() { this._textures = new Array();
      this._timeScale = 1;
      this._animatorBuilder = new LIVE2DCUBISMFRAMEWORK.AnimatorBuilder(); }
    ModelBuilder.prototype.setMoc = function(value) { this._moc = value; return this; };
    ModelBuilder.prototype.setTimeScale = function(value) { this._timeScale = value; return this; };
    ModelBuilder.prototype.setPhysics3Json = function(value) {
      if (!this._physicsRigBuilder) { this._physicsRigBuilder = new LIVE2DCUBISMFRAMEWORK.PhysicsRigBuilder(); }
      this._physicsRigBuilder.setPhysics3Json(value);
      return this;
    };
    ModelBuilder.prototype.setUserData3Json = function(value) {
      if (!this._userDataBuilder) { this._userDataBuilder = new LIVE2DCUBISMFRAMEWORK.UserDataBuilder(); }
      this._userDataBuilder.setUserData3Json(value);
      return this;
    };
    ModelBuilder.prototype.addTexture = function(index, texture) { this._textures.splice(index, 0, texture); return this; };
    ...
    ModelBuilder.prototype.addGroups = function(groups) { this._groups = groups; return this; };
    ModelBuilder.prototype.buildFromModel3Json = function(loader, model3Obj, callbackFunc) {
      ...
      if (typeof(model3Obj.data['FileReferences']['Textures']) !== "undefined") { model3Obj.data['FileReferences']['Textures'].forEach(function(element) { loader.add('texture' + textureCount, modelDir + element);
          textureCount++; }); }
    ...
    ModelBuilder.prototype.build = function() {
      ...
      var physicsRig = null;
      if (this._physicsRigBuilder) { physicsRig = this._physicsRigBuilder.setTarget(coreModel).setTimeScale(this._timeScale).build(); }
      var userData = null;
      if (this._userDataBuilder) { userData = this._userDataBuilder.setTarget(coreModel).build(); }
  ...
  var CubismMesh = (function(_super) {
    ...
    function CubismMesh() { var _this = _super !== null && _super.apply(this, arguments) || this;
      _this.isCulling = false;
      _this.isMaskMesh = false; return _this; }
...
export default LIVE2DCUBISMPIXI
